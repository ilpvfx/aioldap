...
from aioldap.exceptions import (
    LDAPBindException,
        ...
        if len(data) > 0:
            ...
            while self.messages:
                ...
                if (
                    ...
                    logger.debug(
                        "data_received: id {0}, extended response".format(msg_id)
                    )
                    finish = True
                else:
                ...
                if msg_id not in self.responses:
                    ...
                    if finish:
                        ...
                        logger.debug(
                            "data_received: id {0}, marked finished".format(msg_id)
                        )
...
class LDAPConnection(object):
    ...
    async def search(
        ...
        try:
            cookie = resp.additional["controls"]["1.2.840.113556.1.4.319"]["value"][
                "cookie"
            ]
        except KeyError:
            cookie = None
    ...
    async def start_tls(self, ctx: Optional[ssl.SSLContext] = None):
        ...
        if resp.data["description"] != "success":
            raise LDAPStartTlsException(
                "Server doesnt want us to use TLS. {0}".format(resp.data.get("message"))
            )

        await self._proto.start_tls(ctx)

    async def extended(
        self, request_name: str, request_value=None, controls=None, no_encode=None
    ):
        """
    ...
    async def add(
        ...
        attr_object_class = list(
            set([to_unicode(object_class) for object_class in attr_object_class])
        )
        _attributes[object_class_attr_name] = attr_object_class

        add_request = add_operation(
            dn, _attributes, auto_encode, None, validator=None, check_names=False
        )
        msg_id = self._next_msg_id
